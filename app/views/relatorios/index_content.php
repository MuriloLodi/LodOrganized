...
<style>
/* ===== PADRÃO VISUAL (mesmo padrão do dashboard) ===== */
...
.section-sub{ color:#6c757d; font-size:.9rem; }
...
.kpi-value{ font-size:1.55rem; font-weight:900; letter-spacing:-.3px; }
...
<div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-4">
  <div>
    ...
    <div class="page-sub">Central de análises e exportações</div>
  </div>

  <div class="d-flex gap-2 flex-wrap btn-stack">
    <a class="btn btn-outline-secondary"
       href="/financas/public/?url=relatorios&ano=<?= (int)$f['ano'] ?>&mes=<?= (int)$f['mes'] ?>">
      Atualizar
    </a>
...
<?php if (!$hasDompdf): ?>
  <div class="alert alert-warning">
    <strong>PDF desativado:</strong> Dompdf não encontrado no projeto. (CSV funciona normalmente)
  </div>
<?php endif; ?>

<!-- FILTROS -->
<div class="card-soft micro mb-4">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
      <div>
        <div class="section-title">Filtros</div>
        <div class="section-sub">Período padrão: mês selecionado</div>
      </div>
    </div>
  ...
  <div class="col-md-4">
    <div class="card-soft micro h-100">
      <div class="card-body">
        ...
        <div class="text-muted small">Qtd (período): <b><?= (int)$qtd ?></b></div>
      </div>
    </div>
  </div>
  ...
  <div class="col-md-4">
    <div class="card-soft micro h-100">
      <div class="card-body">
        ...
        <div class="text-muted small">Baseado no filtro atual</div>
      </div>
    </div>
  </div>
</div>
...
<div class="row g-3 mb-4">
    ...
    <div class="card-soft micro h-100">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <div class="section-title" style="font-size:1.05rem;">Top categorias</div>
          ...
          <?php foreach($top as $t): ?>
            ...
            <div class="mb-3">
              <div class="d-flex justify-content-between align-items-center">
                <div class="fw-semibold text-truncate" style="max-width:65%;"><?= h($nome) ?></div>
                <div class="fw-semibold <?= $isR?'text-success':'text-danger' ?>">
                  R$ <?= number_format($total,2,',','.') ?>
                </div>
              </div>
              <div class="progress mt-2" style="height:10px;">
                <div class="progress-bar <?= $isR?'bg-success':'bg-danger' ?>" style="width:<?= (float)$w ?>%"></div>
              </div>
            </div>
          <?php endforeach; ?>
        <?php endif; ?>
  ...
  <div class="col-lg-6">
    <div class="card-soft micro h-100">
      <div class="card-body">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <div class="section-title" style="font-size:1.05rem;">Por conta</div>
          <span class="text-muted small">Top 8</span>
        </div>
...
<div class="card-soft micro">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-2">
      <div>
        <div class="section-title" style="font-size:1.05rem;">Lançamentos (preview)</div>
        <div class="text-muted small">A busca abaixo filtra somente na tela</div>
      </div>
      <span class="badge-soft bg-light text-dark">Itens: <?= (int)count($lancamentos ?? []) ?></span>
    </div>

    <div class="table-responsive-mobile">
      <table class="table table-hover align-middle mb-0 row-hover" id="tbl">
        ...
        <?php else: ?>
          <?php foreach($lancamentos as $l): ?>
            <?php
              $qtxt = strtolower(
                (string)($l['descricao'] ?? '').' '.(string)($l['categoria'] ?? '').' '.(string)($l['conta'] ?? '')
              );
            ?>
            <tr data-q="<?= h($qtxt) ?>">
